...
(defn valid? [matrix coords]
  (let [oob-coords (filter #(not (in-bounds? %))
                           coords)]
    (and (empty? oob-coords)
         (not (coords-overlap? coords matrix)))))
...
(defn tick! [delta-time]
  (let [time (:time @state)
        ...
        [px py] (:position t)]
    (when (>= time-since-last-tick 0.25)
      (let [ticked-t (assoc t :position (map + [px py] [0 1]))
        ...
        (if (valid? matrix ticked-coords)
          ...
          (do
            (swap! state update :matrix concat (tetrimino->world-coords t))
